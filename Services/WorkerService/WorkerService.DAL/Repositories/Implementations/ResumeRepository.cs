...
public class ResumeRepository : IResumeRepository
{
    private readonly WorkerDbContext _dbContext;
        ...
        public ResumeRepository(WorkerDbContext dbContext, IConfiguration configuration)
        {
        ...
        {
            var resumesQuery = _dbContext.Resume
                //.Where(resume => !_dbContext.Feedbacks.Any(f => f.resume_id == resume.id && f.status == FeedbackStatus.Accepted))
                .Join(_dbContext.Resume_filter, resume => resume.id, filter => filter.resume_id, (resume, filter) => new { resume, filter })
                //.Join(_dbContext.typeOfActivitie, arg => arg.filter.typeOfActivity_id, activity => activity.id, (arg, activity) => new { arg.resume, arg.filter, activity }) //refactoring this by add httpclient for get activity's name and add cache (redis)
                .AsNoTracking()
                .AsQueryable();
        ...
        {
            var resumesQuery = from resume in _dbContext.Resume
                               where resume.id == id
                               join worker in _dbContext.Worker on resume.worker_id equals worker.id
                               join filter in _dbContext.Resume_filter on resume.id equals filter.resume_id into filterGroup
